...
import { EntityManager, EntityTarget, FindOptionsSelect, FindOptionsWhere, Like, ObjectLiteral, QueryRunner, Repository } from "typeorm";
...
export class BaseRepository<T extends ObjectLiteral> extends Repository<T> {
  ...
  private formatSearch(search: FindOptionsWhere<T>): FindOptionsWhere<T> {
  ...
  async findMany(query: FindManyOptions<T>) {
    ...
    const search: FindOptionsWhere<T> = query.search ? this.formatSearch(query.search as FindOptionsWhere<T>) : {};
